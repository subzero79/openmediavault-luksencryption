...
/*    onCancelButton: function() {
    ...
    onOkButton: function() {
        ...
        OMV.MessageBox.show({
            title: _("Confirmation"),
            msg: _("Do you really want to add this device to crypttab?"),
            buttons: Ext.Msg.YESNO,
            fn: function(answer) {
                if(answer === "no")
                    return;
...
Ext.define("OMV.module.admin.storage.luks.Containers", {
    ...
    columns: [{
            ...
            text: _("Crypttab"),
            sortable: true,
            dataIndex: "crypttabregistration",
            hidden: true,
            stateId: "registered",
            renderer: function(value, metaData, record) {
                value = OMV.util.Format.boolean(value);
...
/*            tbarBtnDisabled["register"] = false;
            ...
            if (true === record.get("unlocked")) {
                ...
                // provide a UUID.
                if(Ext.isEmpty(record.get("luksuuid"))) {
                    tbarBtnDisabled["unlock"] = true;
                    tbarBtnDisabled["header"] = true;
                    tbarBtnDisabled["delete"] = true;
                }
    ...
    onHeaderButton: function(action) {
        var me = this;
        var record = me.getSelected();
        var luksuuid = record.get("luksuuid");
        switch(action) {
            case "backup":
